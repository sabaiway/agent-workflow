...
**Now:** kit 3.13.0 is packed, the tree is clean.
**What I need from you:** one yes on publishing; without it nothing moves.
**What's next:** after your yes — dispatch and smoke.
`;
...
describe('state-block-guard — «nothing needed» is FALSE at Stop, not merely discouraged', () => {
  ...
  for (const slot of banned) {
    it(`rejects a from-you slot reading «${slot.slice(0, 40)}»`, () => {
      const text = GOOD.replace('one yes on publishing; without it nothing moves', slot);
  ...
  it('a slot that merely CONTAINS the word later is fine — only the opening answer is judged', () => {
    const text = GOOD.replace(
      'one yes on publishing; without it nothing moves',
      'confirm the publish; nothing else is needed from you',
    );
    const r = decideStop({ closingText: text });
...
describe('state-block-guard — the announce-and-stop shape', () => {
  ...
  for (const promise of promises) {
    it(`rejects a what-next slot promising imminent work: «${promise}»`, () => {
      const text = GOOD.replace('after your yes — dispatch and smoke', promise);
      const r = decideStop({ closingText: text });
  ...
  it('a CONDITIONAL next step is not an announce — it waits on something named', () => {
    const text = GOOD.replace('after your yes — dispatch and smoke', 'after your approval I take the next class');
    const r = decideStop({ closingText: text });
...
describe('state-block-guard — council folds: the honest phrase must survive the banned one', () => {
  const withFromYou = (slot) => GOOD.replace('one yes on publishing; without it nothing moves', slot);

  // A marker can hide INSIDE a longer word — so raw substring matching flags the most
...
describe('state-block-guard — council folds: a condition excuses only what it precedes', () => {
  const withNext = (slot) => GOOD.replace('after your yes — dispatch and smoke', slot);
...
describe('state-block-guard — named residual: a gate in an earlier comma-clause still excuses', () => {
  ...
  it("does not catch «if the test fails I'll report, and now I start the next class»", () => {
    const text = GOOD.replace('after your yes — dispatch and smoke', "if the test fails I'll report, and now I start the next class");
    const r = decideStop({ closingText: text });
...
describe('state-block-guard — council folds: the English twins must actually be twins', () => {
  ...
  // "at your request" names a PAST request, not a future dependency, so it excused an announce
  // that waits on nothing. Kept out of the gate list rather than qualified — «after your
  // approval» already covers the honest shape.
