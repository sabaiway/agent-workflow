...
- **`/agent-workflow-kit`** (default) — writer — bootstrap; an existing `docs/ai/` → ask upgrade-vs-bootstrap; see *Mode: bootstrap*.
- **`/agent-workflow-kit upgrade`** — writer — upgrade a deployment; see *Mode: upgrade*.
- **`/agent-workflow-kit help`** — read-only — command index + unrecognized-invocation landing; see *Mode: help*.
- **`/agent-workflow-kit gates`** — project-exec — run the project's own declared gates; see *Mode: gates*.
- **`/agent-workflow-kit backends`** — read-only — backend readiness check; see *Mode: backends*.
- **`/agent-workflow-kit setup [backend]`** — writer — place a bridge + link wrappers (opt-in); see *Mode: setup*.
- **`/agent-workflow-kit status`** — read-only — family versions + deployment view; see *Mode: status*.
- **`/agent-workflow-kit recipes`** — read-only — orchestration advisor; see *Mode: recipes*.
- **`/agent-workflow-kit procedures <activity>`** — read-only — activity steps + resolved recipes; see *Mode: procedures*.
- **`/agent-workflow-kit set-recipe`** — writer — orchestration-config writer (previews first); see *Mode: set-recipe*.
- **`/agent-workflow-kit uninstall`** — guarded — teardown; never deletes user content; see *Mode: uninstall*.
- **`/agent-workflow-kit velocity`** — writer — read-only allowlist opt-in; see *Mode: velocity*.
- **`/agent-workflow-kit agents`** — writer — cheap-lane subagents opt-in; see *Mode: agents*.
- **`/agent-workflow-kit hook`** — writer — gate auto-approve hook opt-in; see *Mode: hook*.
- **`/agent-workflow-kit review-state`** — read-only — review-receipt checker; see *Mode: review-state*.
- **`/agent-workflow-kit grounding`** — writer — grounded-review facts assembler; see *Mode: grounding*.
...
- [`tools/`](tools/) — the family-wide tooling the kit **owns and ships**: the `capability.json` schema + validator (`manifest/`), the command catalog + `routeInvocation` router (`commands.mjs`), the per-mode helpers, and the attribution-off `release-scan.mjs`. Each tool's contract lives with the `### Mode:` section that dispatches it and in the tool's own header comment. The bundled bridge skill mirrors live under [`bridges/`](bridges/) (byte-identical, pinned by `test/bridges-mirror.test.mjs`). See [`tools/manifest/schema.md`](tools/manifest/schema.md).
