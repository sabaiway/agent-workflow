...
const RECEIPT_FIXTURE = JSON.parse(
  '{"schema":1,"artifact":"code","fresh":true,"fingerprint":"<sha256hex>","backend":"codex","verdict":"ship","grounded":true,"factsHash":null,"wrapperVersion":"2.2.0","timestamp":"2026-07-08T12:00:00Z","probe":false,"posture":{"model":"<display>"}}',
);
...
// The D3(b) degrade escape reaches --await for FREE via the shared decideCheck: once a
// current-tree degrade RECORD exists in the core-evidence store, --await stops waiting for that
// backend and returns READY (before, it waited forever for a receipt that never comes). Before the
// record exists, or at a stale/old fingerprint, it still waits.
const seedDegradeStore = (fingerprint) => {
  const dir = mkdtempSync(join(tmpdir(), 'await-evidence-'));
  const path = join(dir, 'evidence.jsonl');
  const record = { schema: 1, kind: 'degrade', backend: 'agy', reason: 'Issue-001 stall on a large diff', fingerprint, timestamp: '2026-07-09T00:00:00Z' };
  writeFileSync(path, `${JSON.stringify(record)}\n`);
  return { path, dir };
};

describe('review-state --await — the D3(b) degrade-record escape', () => {
  it('a recorded current-fingerprint degrade → READY immediately (no sleep) — the escape reaches --await', async () => {
    const root = makeRepo();
    const fp = computeTreeFingerprint(root);
    mint(root, { backend: 'codex', fingerprint: fp });
    const { path, dir } = seedDegradeStore(fp);
    let slept = 0;
    const r = await mainAwait(['--await'], { cwd: root, env: { AW_CORE_EVIDENCE: path }, detect: detect(), ...fakeClock(() => { slept += 1; }) });
    rmSync(root, { recursive: true, force: true });
  ...
  it('BEFORE the degrade record exists → still waits (TIMEOUT)', async () => {
    const root = makeRepo();
    const fp = computeTreeFingerprint(root);
    mint(root, { backend: 'codex', fingerprint: fp });
    const r = await mainAwait(['--await', '--timeout', '10'], { cwd: root, env: {}, detect: detect(), ...fakeClock() });
  ...
  it('a degrade at a STALE/old fingerprint → still waits (TIMEOUT) — the degrade must attest THIS tree', async () => {
    ...
    const fp = computeTreeFingerprint(root);
    mint(root, { backend: 'codex', fingerprint: fp });
    const { path, dir } = seedDegradeStore('f'.repeat(64));
    const r = await mainAwait(['--await', '--timeout', '10'], { cwd: root, env: { AW_CORE_EVIDENCE: path }, detect: detect(), ...fakeClock() });
    rmSync(root, { recursive: true, force: true });
    rmSync(dir, { recursive: true, force: true });
