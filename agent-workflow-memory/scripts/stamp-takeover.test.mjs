...
describe('LINEAGE_HEAD', () => {
  it('is the shared deployment-lineage head (not this package version)', () => {
    assert.equal(LINEAGE_HEAD, '4.0.0');
  });
});
...
describe('decideTakeover — §1.5 state table (pure, per row)', () => {
  ...
  it('future .memory-version (> head) → STOP', () => {
    const d = decideTakeover({ memoryVersion: '4.1.0', workflowVersion: null });
    assert.equal(d.status, 'stop');
...
describe('selectMigrations', () => {
  ...
  it('null migrateFrom selects all ≤ head; excludes future migrations', () => {
    assert.deepEqual(selectMigrations(null, [...available, '4.1.0']), ['1.1.0', '1.2.0', '1.3.0']);
  });
  ...
  it('the REAL migrations dir wires a 2.0.0 deployment to exactly the 3.0.0 migration (AD-059)', async () => {
    ...
    const real = (await readdir(migrationsDir))
      ...
      .filter(Boolean);
    assert.deepEqual(selectMigrations('2.0.0', real), ['3.0.0'], 'a 2.0.0-stamped deployment picks up exactly the 3.0.0 migration (memory ships no 4.0.0 note)');
  });
});
