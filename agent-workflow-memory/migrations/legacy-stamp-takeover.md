...
their package versions. The current lineage head is **`4.0.0`** (`LINEAGE_HEAD`). This migration
...
Look at `docs/ai/` and act by row. `V` is the legacy stamp's value; `head` = `4.0.0`.
...
2. **Validate** each value that exists: it must be `MAJOR.MINOR.PATCH`, non-empty, and **not** greater
   than `4.0.0`. If any present stamp is empty, unparseable, or in the future → **STOP and report**;
   do nothing else.
